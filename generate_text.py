...
from transformer_helper import find_latest_checkpoint, load_token_ids, \
    resolve_data_paths, resolve_device
...
def get_first_token_top_probabilities(model, tokenizer, prompt, top_k=10):
    """Get the top probabilities for the first token in the prompt."""
    ctx_tokens = prompt[:, -model.block_size:]

    # TODO: Replace the right hand side of the assignment statement to
    # compute logits from ctx_tokens.
    logits, _ = None, None
    raise IncompleteTodoError() # delete this line when you implement the TODO above
    # TODO: Replace the right hand side of the assignment statement to
    # extract the logits for the next token from the logits tensor.
    # The logits tensor has shape (batch_size, sequence_length, vocab_size),
    # and you want to extract the logits for the last token
    # in the sequence. The resulting next_token_logits tensor should have
...
def generate_text_from_prompt(args, model, tokenizer, prompt, max_new_tokens):
    ...
    with torch.no_grad():
        ...
        # TODO: Replace the right hand side of the assignment statement with code
        # that will generate new tokens from the model, starting with the prompt
        # and producing at most max_new_tokens tokens.
...
def setup_tokenizer_and_prompt(args, device, config, ids_path, meta_path, tokenizer_json_path):
    """Set up the tokenizer and prompt based on command line arguments and data files."""
...
def build_and_load_model(device, checkpoint, config, vocab_size):
    """Build and load the TinyTransformer languagemodel."""
...
def save_and_report_generation(args, checkpoint_path, device, tokenizer_type, prompt_text, generated_text, first_token_top10):
    """Save the generated text and report the generation results."""
    # Print summary
    ...
    if first_token_top10:
        ...
        for rank, (token_id, token_text, prob) in enumerate(first_token_top10, start=1):
            ...
            print(
                f"{rank:2d}. id={token_id:<6d} prob={prob:.6f} ({prob * 100:6.2f}%) token={token_display}")
    print("\nGenerated sample:\n")
...
def load_tokenizer_resources(args, checkpoint, config):
    """Load the tokenizer resources needed for text generation."""
...
def load_checkpoint_and_config(args):
    """Load the checkpoint and config from the command line arguments."""
