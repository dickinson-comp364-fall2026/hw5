...
def apply_config_overrides(config: dict[str, Any], overrides: list[str]) -> list[tuple[str, object]]:
...
def load_experiment_settings(config: dict[str, Any], device: str) -> ExperimentSettings:
    ...
    output = OutputSettings(
        ...
        model_path=(None if output_cfg.get("model_path") is None else str(output_cfg.get("model_path"))),
...
def build_prompt(
    ...
    """Builds a prompt tensor from either provided prompt text or the first `prompt_length` tokens of the training stream. Returns the prompt tensor and the text used to build it.
    Args:
        tokenizer: The Tokenizer object to use for encoding prompt text if provided.
        token_ids: The tensor of token IDs representing the training stream. Shape (stream_length,). Ignored if prompt_text is provided.
    ...
    if prompt_text is not None:
        ...
        print(
            f"Warning: prompt_text '{prompt_text}' encoded to zero tokens. Falling back to first {prompt_length} token(s) of training stream."
        )

    prompt = token_ids[:prompt_length].unsqueeze(0).to(device)
    prompt_text_out = tokenizer.decode(prompt[0].tolist(), skip_special_tokens=False)
