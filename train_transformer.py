...
from transformer_helper import load_token_ids, resolve_data_paths, resolve_device
...
def get_batch(
    ...
    """
    Get a batch of input and target tokens from the data.

    Args:
        data: The input data as a tensor.
        block_size: The size of each block.
        batch_size: The number of blocks in the batch.
...
def load_training_context(args: argparse.Namespace) -> dict[str, object]:
    """Load the training context from the command line arguments."""
...
def print_training_summary(context: dict[str, object]) -> None:
    """Print a summary of the training context."""
...
def run_training_loop(context: dict[str, object]) -> tuple[list[tuple[int, float]], float]:
    ...
    for step in range(1, settings.training.train_steps + 1):
        ...
        # TODO: Add comments stating where and how this loss is computed.
        # Describe what loss function is used and how it works in your own words.
        raise IncompleteTodoError() # delete this line when you implement the TODO above
        _, loss = model(xb, yb)
